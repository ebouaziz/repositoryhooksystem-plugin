...
class RepositoryProxy(object):
    ...
    def set_txn_property(self, property, value):
        if self.txn:
            ...
            raise Exception(
                "can't use set_txn_property outside pre-commit-hook")

    def get_revision_property(self, revision, property):
        propval = fs.svn_fs_revision_prop(
            self.fs,
            revision,
            property,
            self.pool)
    ...
    def get_txn_path_property(self, path, propname):
        ...
        root = self.get_txn_root()
        return fs.svn_fs_node_prop(root, path, propname)
    ...
    def set_revision_log_message(self, revision, msg):
        self.set_revision_property(revision, 'svn:log', msg)
    ...
    def find_revision_branch(self, revision, bcre, strict=False,
        ...
        if not strict:
            return self._find_plain_branch(bcre, paths, btag)
        else:
            return self._find_pure_branch(bcre, paths, btag)

    def find_txn_branch(self, bcre, strict=False, btag='branch'):
    ...
    def _find_pure_branch(self, bcre, chg_paths, btag):
        ...
        branch = path_mo.group(btag)
        return branch and '/%s' % branch

    def _find_plain_branch(self, bcre, chg_paths, btag):
        branch = None
